...
beforeEach(async () => {
	try {
		...
		await pool.query(`INSERT INTO public."user" (id, name, role, company_id, verified, age, hourly_wage)
                      VALUES ($1, $2, $3, $4, $5, $6, $7)`,
			[MOCK_SECOND_OWNER_ID, "Second Owner", UserRole.Owner, MOCK_COMPANY_ID, true, 40, null]);
		await pool.query(`INSERT INTO public."user" (id, name, role, company_id, verified, age, hourly_wage)
                      VALUES ($1, $2, $3, $4, $5, $6, $7)`,
			[MOCK_LEADER_ID, "Seed Leader", UserRole.Leader, MOCK_COMPANY_ID, true, 30, null]);
		await pool.query(`INSERT INTO public."user" (id, name, role, company_id, verified, age, hourly_wage)
                      VALUES ($1, $2, $3, $4, $5, $6, $7)`,
			[MOCK_EMPLOYEE_ID, "Seed Employee", UserRole.Employee, MOCK_COMPANY_ID, true, 25, 15]);
		await pool.query(`INSERT INTO public."user" (id, name, role, company_id, verified, age, hourly_wage)
                      VALUES ($1, $2, $3, $4, $5, $6, $7)`,
			[MOCK_UNVERIFIED_EMPLOYEE_ID, "Unverified Employee", UserRole.Employee, MOCK_COMPANY_ID, false, 22, 14]);
...
describe("Company API Integration Tests", () => {
	...
	describe("GET /company/users", () => {
		...
		it("Owner should get Leader, other Owner, and Employees (excluding self) in their company", async () => {
			...
			expect(userIds).not.toContain(MOCK_OWNER_ID)
			expect(userIds).not.toContain(MOCK_OTHER_EMPLOYEE_ID)

			expect(response.body.data.users[0].role).toBe(UserRole.Owner)
			expect(response.body.data.users[1].role).toBe(UserRole.Leader)
			expect(response.body.data.users[2].role).toBe(UserRole.Employee)
			expect(response.body.data.users[3].role).toBe(UserRole.Employee)
		});
		...
		it("Leader should get Employees (excluding self and Owners/other Leaders) in their company", async () => {
			...
			expect(userIds).toContain(MOCK_EMPLOYEE_ID)
			expect(userIds).toContain(MOCK_UNVERIFIED_EMPLOYEE_ID)
			expect(userIds).not.toContain(MOCK_LEADER_ID)
			expect(userIds).not.toContain(MOCK_OWNER_ID)
			expect(userIds).not.toContain(MOCK_SECOND_OWNER_ID)
		})

		it("should filter users by name (case-insensitive, partial match)", async () => {
			const response = await request(app)
				.get("/company/users?name=seed Le")
				.set("Cookie", OWNER_COOKIE)

			expect(response.status).toBe(200)
			expect(response.body.data.users).toBeInstanceOf(Array)
			expect(response.body.data.users.length).toBe(1)
			expect(response.body.data.users[0].id).toBe(MOCK_LEADER_ID)
			expect(response.body.data.users[0].name).toBe("Seed Leader")
		})

		it("should return paginated results when page=1 is requested", async () => {
			for (let i = 0; i < 25; i++) {
				await pool.query(`INSERT INTO public."user" (id, name, role, company_id, verified, age, hourly_wage)
                          VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6)`,
					[`Extra Employee ${i}`, UserRole.Employee, MOCK_COMPANY_ID, true, 20 + i, 10 + i])
			}

			const response = await request(app)
				.get("/company/users?page=1") // Request page 1 (default limit 20)
				.set("Cookie", OWNER_COOKIE)
			...
			expect(response.body.data.pagination.totalItems).toBe(4 + 25) // Original 4 (excluding self) + 25 new = 29
			expect(response.body.data.pagination.totalPages).toBe(2)
		})

		it("should return second page results when page=2 is requested", async () => {
			for (let i = 0; i < 25; i++) {
				await pool.query(`INSERT INTO public."user" (id, name, role, company_id, verified, age, hourly_wage)
                          VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6)`,
					[`Extra Employee ${i}`, UserRole.Employee, MOCK_COMPANY_ID, true, 20 + i, 10 + i])
			...
			const response = await request(app)
				.get("/company/users?page=2") // Request page 2 (default limit 20)
				.set("Cookie", OWNER_COOKIE)

			expect(response.status).toBe(200)
			expect(response.body.data.users).toBeInstanceOf(Array)
			expect(response.body.data.users.length).toBe(29 - 20) // Remaining users
			expect(response.body.data.pagination).toBeDefined()
			expect(response.body.data.pagination.currentPage).toBe(2)
			expect(response.body.data.pagination.totalItems).toBe(29)
			expect(response.body.data.pagination.totalPages).toBe(2)
		})

		it("should respect the limit query parameter", async () => {
			const response = await request(app)
				.get("/company/users?page=1&limit=3") // Request page 1 with limit 3
				.set("Cookie", OWNER_COOKIE)
		...
		it("should combine name filter and pagination", async () => {
			// Add specific users for filtering
			await pool.query(`INSERT INTO public."user" (id, name, role, company_id, verified, age, hourly_wage)
                        VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6)`,
				[`Filter Target A`, UserRole.Employee, MOCK_COMPANY_ID, true, 21, 11])
			await pool.query(`INSERT INTO public."user" (id, name, role, company_id, verified, age, hourly_wage)
                        VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6)`,
				[`Filter Target B`, UserRole.Employee, MOCK_COMPANY_ID, true, 22, 12])

			const response = await request(app)
				.get("/company/users?page=1&limit=1&name=Filter Target")
				.set("Cookie", OWNER_COOKIE)

			expect(response.status).toBe(200)
			expect(response.body.data.users).toBeInstanceOf(Array)
			expect(response.body.data.users.length).toBe(1)
			expect(response.body.data.users[0].name).toContain("Filter Target")
			expect(response.body.data.pagination).toBeDefined()
